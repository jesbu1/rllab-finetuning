...
from sandbox.finetuning.algos.concurrent_ppo_gradlog import Concurrent_PPO_Gradient_Info
...
from sandbox.finetuning.envs.mujoco.gather.disabled_snake_gather_env import DisabledSnakeGatherEnv
from sandbox.finetuning.envs.mujoco.gather.snake_basin_gather_env import SnakeBasinGatherEnv
from sandbox.finetuning.envs.mujoco.gather.ant_half_gravity_gather_env import HalfGravityAntGatherEnv
from sandbox.finetuning.envs.mujoco.gather.disabled_ant_gather_env import DisabledAntGatherEnv
from sandbox.finetuning.envs.mujoco.gather.ant_hfield_gather_env import AntGatherEnv as AntBasinGatherEnv
...
parser.add_argument('--seed', '-seed', type=int, default=None)  # if you want to run a single, specific random seed
...
parser.add_argument('--reinit_std', '-reinit_std', action='store_true')
...
# for debugging the preference changes
parser.add_argument('--savecomvel', '-scv', action ='store_true')
parser.add_argument('--savejointangles', '-sja', action ='store_true')

# outputs the relative path to the exp directory, to make plotting and syncing easier
...
algo_choices = {'vpg', 'hierarchical_vpg', 'hippo', 'hippo_random_p', 'ppo', 'flatppo',
                    'trpo', 'flattrpo', 'repeatingvpg', 'repeatingppo', 'hippogradinfo'}
env_choices = {'cartpole', 'half_cheetah', 'swimmergather', 'swimmergatherhfield', 'antlowgeargather',
               ...
               'snakegatherhfield', 'snakefixedegoobsgather', 'antlowgeargatherenvcompenalty',
               'antlowgeargatherenvjointpenalty', 'snakegatherenvcompenalty', 'snakegatherenvjointpenalty',
               'snakebasingather', 'snakedisabledgather', 'antbasingather', 'anthalfgravitygather', 'antdisabledgather'}
algo_name = args.algo
...
if args.n_experiments > 0:  # default number of experiments is 10
...
if args.env_name == 'cartpole':
    ...
    if args.period != -1:
        ...
        period = 4
                       ...
                       'snakebasingather', 'snakedisabledgather'}:
    ...
    if args.env_name == 'swimmergather':
        ...
        max_path_length = 8e3
    elif args.env_name == 'snakebasingather':
        env = normalize(SnakeBasinGatherEnv(**keyword_args))
        max_path_length = 8e3
    elif args.env_name == 'snakedisabledgather':
        env = normalize(DisabledSnakeGatherEnv(**keyword_args))
        max_path_length = 8e3
    else:
        raise NotImplementedError
    ...
    elif args.env_name == 'antbasingather':
        env = normalize(AntBasinGatherEnv(**keyword_args))
    elif args.env_name == 'anthalfgravitygather':
        env = normalize(HalfGravityAntGatherEnv(**keyword_args))
    elif args.env_name == 'antdisabledgather':
        env = normalize(DisabledAntGatherEnv(**keyword_args))
    else:
        raise NotImplementedError
    snn_pkl_path = args.snn_path
    assert args.pretrained_manager == (args.manager_path is not None)
...
if algo_name == 'vpg':
    ...
    exp_prefix = "{0}_latdim{1}_period{2}_lr{3}".format(exp_common_prefix, str(latent_dim), str(period),
                                                        str(learning_rate))
elif algo_name == "ppo" or algo_name == "hippo" or algo_name == "hippo_random_p" or algo_name == 'hippogradinfo':
    ...
    if not bilinear_integration:
        exp_common_prefix += "_nobi"
    if algo_name == "ppo" or algo_name == "hippo" or algo_name == 'hippogradinfo':
        policy_params = dict(
            env_spec=env.spec,
            env=env,
            pkl_path=pkl_path,
        ...
        if args.pkl_base_dir is None:
            ...
            elif algo_name == 'hippogradinfo':
                algo = Concurrent_PPO_Gradient_Info(policy=policy, **algo_params)
            else:
    ...
    exp_prefix = "{0}_period{1}_lr{2}_tpi{3}_eps{4}_disc{5}_bs{6}_h{7}".format(exp_common_prefix, str(period),
                                                            str(learning_rate), str(train_pi_iters), str(epsilon),
                                                            str(discount), str(int(batch_size)), str(int(max_path_length)))
# import ipdb; ipdb.set_trace()
assert exp_prefix is not None
if args.reinit_std and args.env_name in {'antlowgeargatherenvcompenalty', 'antlowgeargatherenvjointpenalty',
               'snakegatherenvcompenalty', 'snakegatherenvjointpenalty'}:
    exp_prefix += "_stdreinit"
...
if args.get_exp_paths:
    ...
    if (not args.eval) and (not args.pchange) and (not args.savecomvel) and (not args.savejointangles):
        ...
        if args.eval:
            returns = []
            for i in trange(len(algo_lst), desc="Seed", ncols=80):
                policy = policy_lst[i]
                seed = algo_lst[i][0]
                returns.append(np.mean(eval_p(policy, env, max_path_length, 250, seed)))
